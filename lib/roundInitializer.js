...
module.exports = class RoundInitializer {
    ...
    async tryInitializeRound(roundsManagerAddr, blockNumber) {
        ...
        if (!initialized) {
            console.log(`Round for ${roundsManagerAddr} is not initialized during block ${blockNumber}`)

            if (this.getPendingTxHash(roundsManagerAddr)) {
                console.log(`Round initialization for ${roundsManagerAddr} pending with tx ${this.getPendingTxHash(roundsManagerAddr)}`)
            } else {
                try {
                    await this.initializeRound(roundsManager)
                    
                    const currRound = await roundsManager.currentRound()
                    console.log(`Round ${currRound} initialized for ${roundsManagerAddr}`)
                } catch (err) {
                    console.log(`Error initializing round for ${roundsManagerAddr}: ${err}`)
                }
            }
       } else {
    ...
    async initializeRound(roundsManager) {
        ...
        if (!receipt.status) {
            throw new Error(`Failed to initialize round for ${roundsManager.address}`)
        }
    }
